...
class DatabaseValidationService {
    ...
    try (Connection connection = dataSource.getConnection()) {
      ...
      for (EntityType<?> entity : entityManagerFactory.getMetamodel().getEntities()) {
        ...
        try (ResultSet tables =
          ...
          if (!tables.next()) {
            log.warn(
                "Error: For entity: {}, table {} does not exist in the database!",
  ...
  private void validateColumns(
      EntityType<?> entity, String tableName, Connection conn, DatabaseMetaData metaData)
      throws SQLException {
  ...
  private String getSequenceCheckQuery(String dbProduct) {
    if (dbProduct.contains("postgresql")
        || dbProduct.contains("h2")
        || dbProduct.contains("mysql")) {
      ...
      throw new UnsupportedOperationException(
          "Unknown DB dialect for sequence validation: " + dbProduct);
    }
  }
